...
#ifndef LogisticLossMatlab_H_
#define LogisticLossMatlab_H_
...
class LogisticLossMatlab: public LogisticLoss<L, D> {
public:
...
//				return EXIT_FAILURE;
	...
	virtual void solveLocalProblem(ProblemData<L, D> &instance,
		...
		engEvalString(ep, "alphaInit=alphaInit.*my; ");

		engEvalString(ep,
				"alphaInit =  fmincon(@(alphaInit) LogisticLoss(alphaInit,A,my,mAlpha,mW,lambda,totalN), zeros(size(alphaInit,1),1),[],[],[],[],[-ones(size(alphaInit,1),1)+1e-15, zeros(size(alphaInit,1),1)-1e-15]); ");
